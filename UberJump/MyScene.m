...
-(void)setHero:(int)hero
{
    ...
    switch (_currentLevel) {
        case 1:
            ...
            break;
        case 2:
            levelPlist = [[NSBundle mainBundle] pathForResource: @"Level02" ofType: @"plist"];
            break;
        case 3:
            levelPlist = [[NSBundle mainBundle] pathForResource: @"Level03" ofType: @"plist"];
            break;
        case 4:
            levelPlist = [[NSBundle mainBundle] pathForResource: @"Level04" ofType: @"plist"];
            break;
        case 5:
            levelPlist = [[NSBundle mainBundle] pathForResource: @"Level05" ofType: @"plist"];
            break;
        default:
            levelPlist = [[NSBundle mainBundle] pathForResource: @"Level01" ofType: @"plist"];
            break;
   ...
   //levelPlist = [[NSBundle mainBundle] pathForResource: @"Level01" ofType: @"plist"];
    ...
    for (NSDictionary *platformPosition in platformPositions) {
       ...
       // CGFloat patternX = [platformPosition[@"x"] floatValue];
        ...
        CGFloat patternX = [self convertX:self.frame.size andX:[platformPosition[@"x"] floatValue]];
        CGFloat patternY = [self converY:self.frame.size andY:[platformPosition[@"y"] floatValue]];
        ...
        for (NSDictionary *platformPoint in platformPattern) {
           ...
           // CGFloat y = [platformPoint[@"y"] floatValue];
            CGFloat x = [self convertX:self.frame.size andX:[platformPoint[@"x"] floatValue]];
            CGFloat y = [self converY:self.frame.size andY:[platformPoint[@"y"] floatValue]];
    ...
    for (NSDictionary *starPosition in starPositions) {
        ...
        CGFloat patternX = [self convertX:self.frame.size andX:[starPosition[@"x"] floatValue]];
        CGFloat patternY = [self converY:self.frame.size andY:[starPosition[@"y"] floatValue]];
        ...
        for (NSDictionary *starPoint in starPattern) {
            ...
            CGFloat x = [self convertX:self.frame.size andX:[starPoint[@"x"] floatValue]];
            CGFloat y = [self converY:self.frame.size andY:[starPoint[@"y"] floatValue]];
            
            StarType type = [starPoint[@"type"] intValue];
            
            StarNode *starNode = [self createStarAtPosition:CGPointMake(x + patternX, y + patternY - 50) ofType:type];
    ...
    _tapToStartNode = [SKSpriteNode spriteNodeWithImageNamed:@"TapToStart"];
     if(self.frame.size.height > 568) // Ipad
     {
        _tapToStartNode.position = CGPointMake(self.frame.size.width/2, self.frame.size.height*0.35);
     }
     else
     {
...
-(float)convertX:(CGSize)size andX:(float)X;
{
   
    return  (size.width/320) * X;

}

-(float)converY:(CGSize)size andY:(float)Y
{
    return  (size.height/568) * Y ;
...
- (SKNode *) createBackgroundNode
{
  ...
  for (int nodeCount = 0; nodeCount < 20; nodeCount++) {
    ...
    // Check iphone or Ipad
     if(self.frame.size.height > 568) // Ipad
     {
         node.position = CGPointMake(self.frame.size.width/2, nodeCount* 153.0f);
     }
      else
      {
...
- (SKNode *) createPlayer
{
  ...
  if(self.frame.size.height > 568) // Ipad
  {
      [playerNode setPosition:CGPointMake(self.frame.size.width/2, self.frame.size.height* 0.10f)];
   }
    else
    {
    ...
    NSString * _imagePlayer;
    if(_currentHero ==1)
    {
        _imagePlayer = [NSString stringWithFormat:@"%@",@"Player"];
    
    }
    if(_currentHero ==2)
    {
    
       _imagePlayer = [NSString stringWithFormat:@"%@",@"Player2"];
    }
    
  //SKSpriteNode *sprite = [SKSpriteNode spriteNodeWithImageNamed:@"Player"];
    SKSpriteNode *sprite = [SKSpriteNode spriteNodeWithImageNamed:_imagePlayer];
...
- (void) touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
  ...
  // 4
     ...
     {
          [_player.physicsBody applyImpulse:CGVectorMake(0.0f, 18000.0f)];
     }
    else
    {
...
- (StarNode *) createStarAtPosition:(CGPoint)position ofType:(StarType)type
{
  ...
  if (type == STAR_SPECIAL) {
      ...
      if(i==1)
      {
           ...
           sprite = [SKSpriteNode spriteNodeWithImageNamed:@"SuShi"];
      }
  } else {
      ...
      {
          sprite = [SKSpriteNode spriteNodeWithImageNamed:@"ComNam"];
      }
  }
...
- (SKNode *) createMidgroundNode
{
  ...
  for (int i=0; i<10; i++) {
    ...
    if(self.frame.size.height > 568)
    {
        branchNode.position = CGPointMake(self.frame.size.width/2, self.frame.size.height * 0.9 * i);
    }
    else
    {
...
- (void) update:(CFTimeInterval)currentTime {
  ...
  {
    // TO DO: Open map to select Level
      _currentLevel = _currentLevel +1;
      [GameState sharedInstance].curentLevel = _currentLevel;
...
- (void) didSimulatePhysics
{
  ...
  // Set velocity based on x-axis acceleration
    if(self.frame.size.height > 568)
    {
       _player.physicsBody.velocity = CGVectorMake(_xAcceleration * 200.0f, _player.physicsBody.velocity.dy);
    }
    else
    {
  ...
  if(self.frame.size.height > 568) // Ipad
  {
      ...
      {
          _player.position = CGPointMake(768.0f, _player.position.y);
      }
      else if (_player.position.x > 768.0f)
      {
...
- (void) endGame
{
  ...
  EndGameScene *endGameScene = [[EndGameScene alloc] initWithSize:self.size];
    [endGameScene setLevel:_currentLevel];
    [endGameScene setHeroEndGame:_currentHero];
